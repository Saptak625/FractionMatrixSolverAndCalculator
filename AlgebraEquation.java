        ...
        for(int i=0; i<this.equation.length-1; i++) {
            if(!this.equation[i].equals(new Fraction(0, 1))) {
                ...
                if (this.equation[i].equals(new Fraction(1, 1)) && firstIndex == null) {
                    ...
                    this.equation[AlgebraEquation.size-1] = Fraction.add(this.equation[AlgebraEquation.size-1], Fraction.negate(Fraction.multiply(variableReplacement, this.equation[i])));
                }
            }
        }
        AlgebraEquation.variables[firstIndex] = this.equation[AlgebraEquation.size-1];
    }
}
